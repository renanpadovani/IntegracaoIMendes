...
namespace IntegracaoIMendes.Apresentacao.UI
{
    public partial class ConfiguracoesBDForm : Form
    {
        ...
        private void SalvarCredenciais(string server, string database, string user, string password)
        {
            try
            {
                Domain.DataContext.InfastDataContext _context = new Domain.DataContext.InfastDataContext(server, database, user, password);

                ConnectionDbService.SalvarCredenciais(server, database, user, password);
