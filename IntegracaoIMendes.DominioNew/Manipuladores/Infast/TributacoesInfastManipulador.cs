...
namespace IntegracaoIMendes.Dominio.Manipuladores
{
    public class TributacoesInfastManipulador
    {
            ...
            foreach (Grupos grupo in tribIMendes.Grupos)
            {
                foreach (string ean in grupo.prodEan)
                {
                    ...
                    if (produto != null)
                    {
                        ...
                        tribProd.regras = CriarGrupoProdutoListaRegras(cenario, grupo, tribProd.ProdutoDerivadoID);
        ...
        private List<TributacaoProdutoGrupoLista> CriarGrupoProdutoListaRegras(Cenarios Cenario, Grupos Grupo, Int64 produtoID)
        {
            ...
            foreach (Regra regra in Grupo.Regras)
            {
                foreach (UF uf in regra.UFs)
                {
                    foreach (CaracTrib caracteristica in uf.CFOP.CaracTrib)
                    {
                        ...
                        grupoLista.CodRegimeTributario = ((int)Cenario.CodRegimeTributario).ToString();
                        grupoLista.RegimeTributario = Cenario.RegimeTributario;
